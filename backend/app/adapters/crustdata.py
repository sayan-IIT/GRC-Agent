...
class CrustDataAdapter:
    ...
    async def _call_api(self, vendor_id: str) -> dict[str, Any]:
        ...
        params = {
            "fields": "company_name,company_id,headcount,total_investment_usd,web_traffic,news_articles,job_openings"
        }
    ...
    def normalize_signals(self, raw_data: dict) -> list:
        ...
        funding = self._as_float(company.get("total_investment_usd") or company.get("funding_total_usd"), 25_000_000.0)
        ...
        operational_value = max(0.05, min(0.95, 1 - (headcount / 500)))
        trend_value = max(0.05, min(0.95, 0.45 + (news_count * 0.04) - (job_count * 0.025)))
